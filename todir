...
def extract_names(filename):
    ...
    for line in file:   ## iterates over the lines of the file
        match = re.search(r'(name="year"\sid="yob".*)([\d]{4})', line)
        if match:
            year = match.group(2)    
        
        match = re.search(r'(<td>)([\d]*)(</td>)(<td>)([\w]*)(</td>)(<td>)([\w]*)(</td>)', line)
        if match:            
            names_dict[match.group(5)] = match.group(2)
            names_dict[match.group(8)] = match.group(2)
...
def main():
    ...
    for filename in args:
        year, names_dict = extract_names(filename)
